...
struct treeNode { 
    ...
    int chain; // Chain of this node
} node[N];
 
struct Edge { // Every Edge has a weight and two ends. We store deeper end 
    int weight;  
    int deeper_end; 
} edge[N];
 

struct segmentTree { // we construct one segment tree, on base array 
...
// curr is the current node, prev is the parent of curr, dep is its depth
...
void hld(int curr_node, int id, int *edge_counted, int *curr_chain, int n, int chain_heads[]){
    // if the current chain has no head, this node is the first node and also chain head 
    ...
    // set position of node in the array acting as the base to the segment tree 
    ...
    // for every other normal child, do HLD on child subtree as separate chain
    for (int j=0; j<n; j++) {
...
int construct_ST(int ss, int se, int si){
    ...
    // If there are more than one elements, then recur for left and right subtrees and store the maximum of two values in this node
    int mid = (ss + se)/2;
...
int LCA(int u, int v, int n) {
    ...
    int LCA_aux[n+5]; // array for storing path from u to root 
 
    if (node[u].depth < node[v].depth)
       swap(u, v);
 
    memset(LCA_aux, -1, sizeof(LCA_aux)); // LCA_aux will store path from node u to the root
    ...
    /* find first node common in path from v to root and u to root using LCA_aux */
    while (v) {
...
/*  A recursive function to get the maximum value in a given range of array indexes. The following are parameters for this function.
...
int RMQUtil(int ss, int se, int qs, int qe, int index) { 
    if (qs <= ss && qe >= se-1) // If segment of this node is a part of given range
        return s.tree[index];
 
    if (se-1 < qs || ss > qe) // If segment of this node is outside the given range
        return -1;
...
int RMQ(int qs, int qe, int n) {
    ...
    if (qs < 0 || qe > n-1 || qs > qe){ // Check for erroneous input values
...
// we move to the surface changing u and chains until u and v donot belong to the same
int crawl_tree(int u, int v, int n, int chain_heads[]) {
    ...
    while (true) {
        ...
        // if the two nodes belong to same chain, we can query between their positions in the array acting as base to the segment tree. After the RMQ, we can break out as we have no where further to go 
        if (chain_u==chain_v) {
            if (u == v); // trivial -> we exit from the loop and ans = 0
            else
                ans = max(RMQ(node[v].pos_segbase+1, node[u].pos_segbase, n), ans);
            break;
        }else{// else, we query between node u and head of the chain to which u belongs and later change u to parent of head of the chain to which u belongs indicating change of chain 
...
int main() {
    ...
    memset(tree, -1, sizeof(tree)); // fill adjacency matrix with -1 to indicate no connections
    ...
    int root = 0, parent_of_root=-1, depth_of_root=0; // Note that the root is the node 1, that is 0 in the matrix
 
    // a DFS on the tree to set up: arrays for parent, depth, subtree size for every node, deeper end of every Edge 
    dfs(root, parent_of_root, depth_of_root, n);
 
    int chain_heads[N]; // we have at most N chain, one for each node
 
    // we have initialized no chain heads 
    memset(chain_heads, -1, sizeof(chain_heads));
 
    // Stores number of edges for construction of segment tree. Initially we haven't traversed any Edges.
    int edge_counted = 0;
    ...
    // Segment tree of segregated Edges 
    construct_ST(0, edge_counted, 1);
 
    // Since indexes are 0 based, node 11 means index 11-1, 8 means 8-1, and so on
    int u = 11, v  = 9;
